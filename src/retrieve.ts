...
customSDK = new PumpSwapSDK("mainnet");
...
export async function closeAcc() {
	...
	try {
		...
		const pumpSwapPool = await customSDK?.getPumpSwapPool(marketIdPubkey);
		if (pumpSwapPool) {
			isPumpSwap = true;
...
export const sell_pump_amm = async (base_mint: PublicKey, keypair: Keypair, tokenAmount: number, wsolAmount: number) => {
	try {
		...
		const pool = await customSDK?.getPumpSwapPool(base_mint);

		if (!pool) {
			console.log("No PumpSwap pool found for token:", base_mint.toString());
			return null;
		}

		// Calculate amounts
		const base_amt = new BN(Math.floor(tokenAmount * 10 ** 6)); // Convert to micro units
		const sellTokenAmount = Math.floor(Number(base_amt.toString()));
		console.log("sellTokenAmount:", sellTokenAmount);

		// Get the token accounts
		const tokenAcc = await spl.getAssociatedTokenAddress(base_mint, keypair.publicKey);
		const wsolAcc = await spl.getAssociatedTokenAddress(NATIVE_MINT, keypair.publicKey);

		// Build swap instruction
		const sellIxs = await pSwap?.swapBaseInstructions(
			pool,
			new BN(sellTokenAmount),
			2.0, // Slippage tolerance
			"baseToQuote" as Direction,
			keypair.publicKey, // Using keypair's public key as owner
		...
		if (!sellIxs) {
			console.log("Failed to create PumpSwap sell instructions");
			return null;
		}

		// Return the array directly - this is the key change
		return sellIxs;
	} catch (error) {
		console.log("Error in sell_pump_amm:", error);
		return null;
	}
};
...
export const closePumpSwapAcc = async (keypairs: Keypair[], baseMint: PublicKey) => {
	for (let index = 0; index < keypairs.length; index++) {
		...
		const tokenAcc = await spl.getAssociatedTokenAddress(baseMint, keypair.publicKey);
		const wsolAcc = await spl.getAssociatedTokenAddress(spl.NATIVE_MINT, keypair.publicKey);
		...
		if (accinfo) {
			...
			if (!sellInstruction) {
				console.log("Failed to create sell instructions for PumpSwap");
				continue;
			}
		...
		const versionedTx = new VersionedTransaction(message);
		versionedTx.sign([wallet, keypair]); // Sign with both wallet and keypair

		try {
			// Simulate before sending
			const swapsim = await connection.simulateTransaction(versionedTx, { sigVerify: true });

			if (swapsim.value.err) {
				console.log("Transaction simulation failed:", swapsim.value.err);
				continue;
			}

			const swapId = await connection.sendTransaction(versionedTx);
			const lockPoolConfirm = await connection.confirmTransaction(swapId);
			console.log(`Transaction confirmed: ${swapId}`);
		} catch (e) {
			console.log("Error with versionedTx:", e);
		}
	}
};
